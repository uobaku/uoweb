...
const countUp = (el, target) => {
    ...
    const updateCounter = () => {
        count += speed;
        if (count >= target) {
            el.innerText = target + (el.classList.contains('year') ? '' : '+');
            el.nextElementSibling.style.opacity = 1;  // Show the plus sign
        } else {
            el.innerText = Math.floor(count) + (el.classList.contains('year') ? '' : '+');
...
document.addEventListener('wheel', (e) => {
    ...
    scrollTimeout = setTimeout(() => {
        ...
        // Run the animation on each scroll completion
        triggerAnimations(sections[currentSectionIndex]);
    }, 100); // Wait for 100ms after the last scroll event
});

// Add touch support for touchpad and mobile devices
let startY = 0;
let endY = 0;
const swipeThreshold = 50; // Define a swipe threshold
...
document.addEventListener('touchend', () => {
    ...
    if (swipeDistance > swipeThreshold && currentSectionIndex < sections.length - 1) {
        currentSectionIndex++;
    } else if (swipeDistance < -swipeThreshold && currentSectionIndex > 0) {
        currentSectionIndex--;
    }
    scrollToSection(currentSectionIndex);
